...
const _checkConversationCompletionAndQueueGpt = async (
  ...
  try {
    ...
    if (!conversation) {
      throw new Error(`Conversation ${conversationId} not found during completion check`);
    }
    ...
    if (transcribedCount === requiredAudios) {
      log.debug(`Updating conversation status to processing`, { conversationId });
...
const cleanupOnFailure = async (audioPath: string): Promise<void> => {
  try {
    ...
    log.error(`Failed to cleanup audio file`, { audioPath, error: formatError(error) });
  }
};
...
const processAudio = async (job: Job<AudioJob>): Promise<void> => {
  ...
  try {
    ...
    log.debug(`Transcription completed`, { audioId, transcriptionLength: transcription?.length || 0 });
    ...
    log.info(`Audio job completed`, { audioId, duration: totalDuration, jobId: job.id, conversationId });
  } catch (error) {
    ...
    try {
      ...
      log.error(`Failed to update audio status after error`, { audioId, error: formatError(updateError) });
    }
...
worker.on('active', job => log.info(`Audio job started processing`, { jobId: job.id }));
worker.on('completed', job => log.info(`Audio job completed successfully`, { jobId: job.id }));
worker.on('failed', (job, err) => log.error(`Audio job failed`, { jobId: job?.id, error: err.message }));
worker.on('stalled', jobId => log.error(`Audio job stalled`, { jobId }));
worker.on('error', error => log.error(`Audio worker error`, { error: error.message }));
