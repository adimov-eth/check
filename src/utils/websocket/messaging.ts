...
export async function sendBufferedMessages(ws: WebSocketClient, topic: string): Promise<void> {
    ...
    log.info(`[sendBufferedMessages] User ${ws.userId} checking Redis buffer: ${key}`);

    try {
        const messages = await redisClient.lRange(key, 0, -1);
        if (messages.length === 0) {
            log.info(`[sendBufferedMessages] No buffered messages found in Redis for ${key}`);
            return;
        }

        log.info(`[sendBufferedMessages] Found ${messages.length} raw messages in Redis for ${key}. Processing...`);
        ...
        for (const msgString of messages) {
            try {
                ...
                if (!msg.data || !msg.timestamp) {
                    log.warn(`[sendBufferedMessages] Parsed message from ${key} is missing 'data' or 'timestamp'. Skipping.`);
                    parseErrorCount++;
                    continue;
                }
                ...
                log.error(`[sendBufferedMessages] Failed to parse buffered message from Redis (${key}): ${parseError}. Raw: ${msgString.substring(0,100)}...`);
                parseErrorCount++;
            }
        }

        log.info(`[sendBufferedMessages] Processing ${messagesToSend.length} valid, non-expired messages for ${key} (Expired: ${expiredCount}, Parse Errors: ${parseErrorCount})`);

        for (const msgData of messagesToSend) {
            log.debug(`[sendBufferedMessages] Sending buffered message type ${msgData.type} to user ${ws.userId} for topic ${topic}`);
            if (ws.readyState === WebSocket.OPEN) {
                try {
                    ws.send(JSON.stringify(msgData));
                    sentCount++;
                } catch (sendError) {
                    log.error(`[sendBufferedMessages] Error sending buffered message type ${msgData.type} to ${ws.userId}: ${sendError}`);
                    skippedCount++;
                }
            } else {
                skippedCount++;
...
export async function bufferMessage(userId: string, topic: string, data: WebSocketMessage): Promise<void> {
    ...
    try {
        ...
        log.debug(`Buffered message for user ${userId}, topic ${topic}. Redis list size: ${listLength}`);
    } catch (redisError) {
...
export function sendToUser(userId: string, data: WebSocketMessage): void {
    ...
    if (!userClients || userClients.size === 0) {
        log.debug(`No active clients for user ${userId}, cannot send message directly.`);
    ...
    userClients.forEach((client) => {
        if (client.userId === userId && !client.isAuthenticating && client.readyState === WebSocket.OPEN) {
            try {
                ...
                log.error(`Error sending direct message to user ${userId} (client instance): ${error}`);
                // Optionally: Handle client termination on send error
                // client.terminate();
            }
        }
    });
    log.debug(`Sent direct message to ${sentCount}/${userClients.size} clients for user ${userId}`);
}
...
export function broadcast(data: WebSocketMessage): void {
    const wss = getWss();
    if (!wss) {
        log.warn('Attempted to broadcast message but WebSocket server is not initialized');
        return;
    }
    ...
    wss.clients.forEach((client) => {
        ...
        if (wsClient.userId && !wsClient.isAuthenticating && wsClient.readyState === WebSocket.OPEN) {
            try {
                ...
                log.error(`Error broadcasting message to client ${wsClient.userId}: ${error}`);
                errorCount++;
                // Optionally terminate client on send error
                // wsClient.terminate();
            }
    ...
    log.debug(`Broadcast message: Sent to ${sentCount} authenticated clients. Errors: ${errorCount}. Total connected: ${totalClients}.`);
}